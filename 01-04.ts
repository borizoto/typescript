...
//• bIndex – number
//The function then swaps the element at index aIndex in a with the element at index bIndex in b
//The function doesn’t need to return anything; it directly mutates the arrays.

function swap<Type>(a: Type[], aIndex: number, b: Type[], bIndex: number): void {
    const temp = a[aIndex];
    a[aIndex] = b[bIndex];
    b[bIndex] = temp;
}
